...
@section('content')
    <div class="card">
        <div class="card-body">
            <form action="{{ route('salary.update',$salary->id) }}" method="post" id="edit-form" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <select name="year" class="form-control select-year">
                            ...
                            @for ($i = 0; $i < 10; $i++)
                            <option value="{{now()->addYears(5)->subYears($i)->format('Y')}}" @if($salary->year == now()->addYear(5)->subYears($i)->format('Y')) selected @endif>
